...
/* DESIGN VARIANT 01 — Modern Gold */
.brand-title{font-family:"Trebuchet MS",Arial,sans-serif;font-weight:900;letter-spacing:-1.1px}
.brand-sub,.datetime,.product,.thead,.rules-title,.rule p,.prices{font-family:"Trebuchet MS",Arial,sans-serif}
.gold-mark .bar{background:linear-gradient(145deg,#fff7b5 0%,#ffd84d 26%,#e7a400 58%,#b96b00 82%,#ffe77a 100%);border:1px solid rgba(150,93,0,.42);box-shadow:0 3px 5px rgba(132,80,0,.20)}
.gold-mark .bar-a{transform:skewX(-16deg) rotate(-1deg)}
.gold-mark .bar-b{transform:skewX(-16deg) rotate(1deg)}
.gold-mark .bar-c{transform:skewX(-16deg) rotate(-1deg)}
.gold-mark .purity{font-family:Arial,sans-serif;text-shadow:0 1px rgba(255,231,122,.45)}
.gold-heading{letter-spacing:-.25px}
