...
<style>
:root{--gold:#c59a22;--gold2:#9f7a14;--black:#070707;--white:#fff;--text:#171717;--muted:#666;--line:#eadfbf}
*{box-sizing:border-box}
html,body{margin:0;padding:0}
body{min-height:100vh;background:linear-gradient(145deg,#f8f8f8 0%,#fffdf5 52%,#f1f1f1 100%);font-family:Arial,Helvetica,sans-serif;color:var(--text);padding:22px 14px 30px}
.dashboard{width:min(900px,100%);margin:auto}
.date-card{background:rgba(255,255,255,.96);border:1px solid var(--gold);border-radius:20px;padding:20px 18px 17px;text-align:center;box-shadow:0 12px 32px rgba(0,0,0,.08)}
.date{font-size:clamp(34px,6vw,58px);line-height:1;font-weight:800;letter-spacing:.2px}
.live-row{display:flex;align-items:center;justify-content:center;gap:9px;margin:13px 0 22px;font-size:clamp(17px,3vw,24px);font-weight:800;letter-spacing:.15px}
.hk-flag{width:25px;height:17px;display:inline-flex;flex:0 0 25px;vertical-align:middle;overflow:hidden;border-radius:2px;box-shadow:0 1px 3px rgba(0,0,0,.18)}
.hk-flag svg{width:100%;height:100%;display:block}
.quote-card{overflow:hidden;border-radius:18px;background:var(--black);box-shadow:0 15px 34px rgba(0,0,0,.20)}
.quote-head,.quote-row{display:grid;grid-template-columns:1fr 1.28fr;align-items:center}
.quote-head{background:linear-gradient(135deg,var(--gold),var(--gold2));color:#fff;font-weight:800;font-size:clamp(18px,3vw,25px);padding:17px 20px}
.quote-row{color:#fff;padding:17px 20px;min-height:68px;border-top:1px solid rgba(255,255,255,.18)}
.product{font-size:clamp(17px,2.8vw,24px);font-weight:800}
.prices{text-align:right;font-size:clamp(22px,4vw,33px);font-weight:800;white-space:nowrap;letter-spacing:.2px}
...
.notes-title-row{display:flex;align-items:center;gap:10px;margin-bottom:12px}
.notes-line{height:1px;background:linear-gradient(90deg,transparent,var(--gold));flex:1}
.notes-line.right{background:linear-gradient(90deg,var(--gold),transparent)}
.notes-title{background:linear-gradient(135deg,var(--gold),var(--gold2));color:#fff;border-radius:999px;padding:9px 18px;font-size:18px;font-weight:800;white-space:nowrap;box-shadow:0 5px 14px rgba(159,122,20,.16)}
.notes-card{background:rgba(255,255,255,.94);border:1px solid var(--line);border-left:3px solid var(--gold);border-radius:16px;padding:3px 22px;box-shadow:0 9px 24px rgba(0,0,0,.06)}
.note{display:flex;gap:12px;padding:14px 0;font-size:16px;line-height:1.48;color:#303030}
.note+.note{border-top:1px dashed #d9c98e}
.note-dot{width:8px;height:8px;margin-top:7px;flex:0 0 8px;border-radius:50%;background:var(--gold)}
.note b{color:#111}
.footer{display:flex;align-items:center;justify-content:center;gap:9px;margin-top:20px;color:var(--muted);font-size:14px}
.footer-icon{width:27px;height:27px;border:2px solid var(--gold);border-radius:50%;position:relative}
.footer-icon:before{content:"";position:absolute;width:2px;height:7px;background:var(--gold);left:11px;top:5px}
...
@media(max-width:620px){
 body{padding:14px 9px 22px}.date-card{padding:17px 10px 15px;border-radius:16px}
 .live-row{margin:11px 0 17px}.quote-head{padding:14px 12px}.quote-row{padding:15px 12px;min-height:64px}
 .prices{font-size:clamp(19px,5vw,28px)}.product{font-size:clamp(16px,4.2vw,22px)}
 .notes-section{margin-top:25px}.notes-title{font-size:16px;padding:8px 14px}.notes-card{padding:2px 15px}.note{font-size:15px;padding:13px 0;line-height:1.44}
}
@media(max-width:420px){
 .quote-head,.quote-row{grid-template-columns:1fr 1.38fr}.quote-head{font-size:16px}.prices{font-size:19px}.product{font-size:16px}
 .notes-title-row{gap:6px}.notes-title{font-size:14px;padding:7px 11px}.note{font-size:14px}
}
...
</style></head><body><main class="dashboard"><section class="date-card"><div id="date" class="date">--</div></section><div class="live-row"><span class="hk-flag" aria-label="Hong Kong flag">
<svg viewBox="0 0 24 16" role="img" aria-hidden="true">
<rect width="24" height="16" rx="1.2" fill="#DE2910"/>
<g transform="translate(12 8)" fill="#fff">
<path d="M0-5.2C1.5-3.4 3.5-3.2 4.7-4c-.5 2 .4 3.3 1.9 3.9C4.6.4 3.9 2 4.6 3.7 2.9 3 1.2 3.5 0 5.2-1.2 3.5-2.9 3-4.6 3.7-3.9 2-4.6.4-6.6-.1-5.1-.7-4.2-2-4.7-4-3.5-3.2-1.5-3.4 0-5.2Z"/>
<circle r="1.15" fill="#DE2910"/>
</g></svg></span><span id="time">--:--:-- -- HKT</span></div><section class="quote-card"><div class="quote-head"><div>Product (999.9)</div><div style="text-align:right">USD Rate / Gram</div></div><div id="rows"></div></section><section class="notes-section"><div class="notes-title-row"><div class="notes-line"></div><div class="notes-title">📝 Booking Rule:</div><div class="notes-line right"></div></div><div class="notes-card"><div class="note"><span class="note-dot"></span><span>After checking the live rate, whenever you're ready to confirm a booking, you must write down the <b>required grams</b>.</span></div><div class="note"><span class="note-dot"></span><span>Once you send the grams, we will reply with <b>"OK"</b> at that same time, confirming your booking is being processed at that exact moment.</span></div><div class="note"><span class="note-dot"></span><span>Right after confirmation, we will send you a <b>screenshot of the rate</b>.</span></div><div class="note"><span class="note-dot"></span><span>If your booking is urgent, please <b>call us directly</b> to alert us at the time of booking — WhatsApp messages may sometimes be missed or delayed, so a call ensures immediate confirmation.</span></div></div></section><div class="footer"><span class="footer-icon"></span><span>Prices update in real-time</span></div></main>
